...
function mf_tournaments_pgn_db($event_id, $round_no = false, $brett_no = false, $tisch_no = false) {
	...
	$sql = 'SELECT partien.partie_id
			...
			, IF(ISNULL(url), IF(LOCATE("&contacts_virtual=1", place_categories.parameters), (SELECT identification FROM eventdetails WHERE eventdetails.event_id = events.event_id AND active = "yes" LIMIT 1), place), url) AS Site
