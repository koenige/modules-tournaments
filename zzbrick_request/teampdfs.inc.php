 ...
 * @copyright Copyright © 2013-2014, 2017-2026 Gustaf Mossakowski
...
function mf_tournaments_pdf_teams($event, $params) {
	...
	foreach (array_keys($teams) as $team_id) {
		...
		if (!empty($params['check_uploads'])) {
			$filename = sprintf('%s/teampdfs/%s%%s.pdf', wrap_setting('tournaments_dir'), $teams[$team_id]['team_identifier']);
			$filenames = [
				sprintf($filename, ''),
...
function mod_tournaments_teampdfs_bookings(&$pdf, $team, $settings) {
	...
	$pdf->Ln();
	$pdf->setFont('DejaVu', '', $settings['font_size']);
	if (empty($team['bookings'])) $team['bookings'] = [];
	foreach ($team['bookings'] as $line) {
		...
		$y_pos = $pdf->GetY();
		$x_pos = $pdf->GetX();
		$pdf->MultiCell(201, 14, $line['booking'], 'T', 'L', 1);
		if ($line['anmerkungen']) {
			$pdf->setX($x_pos);
			$pdf->setFont('DejaVu', '', $settings['font_size_remarks']);
		...
		$pdf->SetY($y_pos);
		$pdf->SetX($x_pos + 201);
		$pdf->Cell(79, 14, wrap_money($line['price']).' '.$line['betrag_waehrung'], 'T', 0, 'R');
