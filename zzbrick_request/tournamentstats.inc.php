...
function mod_tournaments_tournamentstats($vars) {
	...
	$sql = 'SELECT event_id
			...
			, (SELECT COUNT(*) FROM participations LEFT JOIN persons USING (person_id) WHERE event_id = events.event_id AND persons.sex = "male" AND teilnahme_status = "Teilnehmer" AND (NOT ISNULL(brett_no) OR ISNULL(team_id)) AND usergroup_id = %s) AS tn_m
