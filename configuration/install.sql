...
CREATE OR REPLACE VIEW `paarungen_ergebnisse_view` AS
	...
	UNION SELECT `paarungen`.`event_id`, `paarungen`.`auswaerts_team_id` AS `team_id`, `teams`.`team_id` AS `gegner_team_id`, `paarungen`.`runde_no`, 1 AS `kampflos`, `tournaments`.`bretter_min` AS `brettpunkte`, 0 AS `bp_opponent`, 2 AS `mannschaftspunkte`, 0 AS `mp_opponent`
	FROM `paarungen`
	JOIN `teams`
...
CREATE OR REPLACE VIEW `buchholz_mit_kampflosen_view` AS
	...
	LEFT JOIN `tabellenstaende_termine_view`
		ON `paarungen_ergebnisse_view`.`team_id` = `tabellenstaende_termine_view`.`team_id`
		AND `paarungen_ergebnisse_view`.`runde_no` <= `tabellenstaende_termine_view`.`runde_no`
	...
	GROUP BY `tabellenstaende_termine_view`.`team_id`, `tabellenstaende_termine_view`.`runde_no`
	UNION SELECT `tabellenstaende_termine_view`.`runde_no`, `tabellenstaende_termine_view`.`team_id`, (SUM(`bisherige_paarungen`.`mannschaftspunkte`) + (`tabellenstaende_termine_view`.`runde_no` - `paarungen_ergebnisse_view`.`runde_no`)) AS `buchholz_kampflos_mit_korrektur`
	FROM `paarungen_ergebnisse_view` `bisherige_paarungen`
	LEFT JOIN `paarungen_ergebnisse_view`
		ON `bisherige_paarungen`.`team_id` = `paarungen_ergebnisse_view`.`team_id`
		AND `bisherige_paarungen`.`runde_no` < `paarungen_ergebnisse_view`.`runde_no`
	LEFT JOIN `tabellenstaende_termine_view`
		ON `paarungen_ergebnisse_view`.`team_id` = `tabellenstaende_termine_view`.`team_id`
		AND `paarungen_ergebnisse_view`.`runde_no` <= `tabellenstaende_termine_view`.`runde_no`
	WHERE `paarungen_ergebnisse_view`.`kampflos` = 1
	GROUP BY `tabellenstaende_termine_view`.`team_id`, `tabellenstaende_termine_view`.`runde_no`, `paarungen_ergebnisse_view`.`runde_no`
	UNION SELECT `tabellenstaende_termine_view`.`runde_no`, `tabellenstaende_termine_view`.`team_id`, (`tabellenstaende_termine_view`.`runde_no` - `paarungen_ergebnisse_view`.`runde_no`) AS `buchholz_kampflos_mit_korrektur`
	FROM `tabellenstaende_termine_view`
