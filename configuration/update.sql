...
/* 2026-07-27-1 */	INSERT INTO categories (`category`, `description`, `main_category_id`, `path`, `parameters`, `sequence`, `last_update`) SELECT 'Tournaments', NULL, NULL, 'tournaments', '&alias=tournaments&use_subtree=1', NULL, NOW();
/* 2026-07-27-2 */	UPDATE categories SET category = 'Status', main_category_id = /*_ID categories tournaments _*/, path = 'tournaments/status', parameters = '&alias=tournaments/status&own_type_category=1&form_display=set&min_records=1&max_records=10' WHERE path IN ('turnierstatus', 'tournament-status');
/* 2026-07-27-3 */	UPDATE categories SET path = CONCAT('tournaments/status/', SUBSTRING_INDEX(path, '/', -1)) WHERE path LIKE 'turnierstatus/%' OR path LIKE 'tournament-status/%';
/* 2026-07-27-4 */	RENAME TABLE `turniere_status` TO `tournaments_categories`;
/* 2026-07-27-5 */	ALTER TABLE `tournaments_categories` CHANGE `turnier_status_id` `tournament_category_id` int unsigned NOT NULL AUTO_INCREMENT, CHANGE `status_category_id` `category_id` int unsigned NOT NULL;
/* 2026-07-27-6 */	ALTER TABLE `tournaments_categories` ADD `property` varchar(255) COLLATE 'utf8mb4_unicode_ci' DEFAULT NULL AFTER `category_id`, ADD `type_category_id` int unsigned NULL AFTER `property`, ADD `sequence` tinyint unsigned NULL AFTER `type_category_id`, ADD `last_update` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `sequence`;
/* 2026-07-27-7 */	UPDATE tournaments_categories SET type_category_id = (SELECT category_id FROM categories WHERE path = 'tournaments/status' LIMIT 1);
/* 2026-07-27-8 */	ALTER TABLE `tournaments_categories` MODIFY `type_category_id` int unsigned NOT NULL;
/* 2026-07-27-9 */	ALTER TABLE `tournaments_categories` DROP INDEX `turnier_id_status_kategorie_id`, ADD UNIQUE `tournament_id_category_id` (`tournament_id`,`category_id`), ADD INDEX `category_id` (`category_id`), ADD INDEX `type_category_id` (`type_category_id`);
/* 2026-07-27-10 */	UPDATE _relations SET `detail_table` = 'tournaments_categories', `detail_id_field` = 'tournament_category_id' WHERE `detail_table` = 'turniere_status';
/* 2026-07-27-11 */	UPDATE _relations SET `detail_field` = 'category_id' WHERE `detail_table` = 'tournaments_categories' AND `detail_field` = 'status_category_id';
/* 2026-07-27-12 */	INSERT INTO _relations (`master_db`, `master_table`, `master_field`, `detail_db`, `detail_table`, `detail_id_field`, `detail_field`, `delete`) SELECT (SELECT DATABASE()), 'categories', 'category_id', (SELECT DATABASE()), 'tournaments_categories', 'tournament_category_id', 'type_category_id', 'no-delete';
/* 2026-07-29-1 */	INSERT INTO categories (`category`, `description`, `main_category_id`, `path`, `parameters`, `sequence`, `last_update`) SELECT 'Tournament Uploads', NULL, NULL, 'tournament-uploads', '&alias=tournament-uploads', NULL, NOW();
/* 2026-07-29-2 */	INSERT INTO categories (`category`, `description`, `main_category_id`, `path`, `parameters`, `sequence`, `last_update`) SELECT 'Registration form', 'Upload the completed, scanned registration form', /*_ID categories tournament-uploads _*/, 'tournament-uploads/registration-form', '&alias=tournament-uploads/registration-form&tournaments_field_name=registration-form&optional=1', 1, NOW();
/* 2026-08-11-1 */	UPDATE _settings SET setting_key = 'tournaments_pgn_bulletin_dir' WHERE setting_key = 'pgn_bulletin_dir';
/* 2026-08-11-2 */	UPDATE _settings SET setting_key = 'tournaments_pgn_dir' WHERE setting_key = 'pgn_dir';
/* 2026-08-11-3 */	UPDATE _settings SET setting_key = 'tournaments_pgn_live_dir' WHERE setting_key = 'pgn_live_dir';
/* 2026-08-11-4 */	UPDATE _settings SET setting_key = 'tournaments_pgn_queue_dir' WHERE setting_key = 'pgn_queue_dir';
/* 2026-08-11-5 */	UPDATE _settings SET setting_key = 'tournaments_pgn_match_round_table_board' WHERE setting_key = 'pgn_match_round_table_board';
/* 2026-08-11-6 */	UPDATE _settings SET setting_key = 'tournaments_pgn_preparation_function' WHERE setting_key = 'pgn_preparation_function';
/* 2026-08-11-7 */	UPDATE categories SET parameters = REPLACE(parameters, '&optional=1', '&tournaments_upload_optional=1') WHERE parameters LIKE '%&optional=1%';
/* 2026-08-11-8 */	UPDATE categories SET parameters = REPLACE(parameters, '&show_main_tournament_archive=', '&tournaments_show_main_archive=') WHERE parameters LIKE '%&show_main_tournament_archive=%';
/* 2026-08-11-9 */	UPDATE categories SET parameters = REPLACE(parameters, '&playermessages=', '&tournaments_playermessages=') WHERE parameters LIKE '%&playermessages=%';
/* 2026-08-11-10 */	UPDATE categories SET parameters = REPLACE(parameters, '&home_team_first_board=', '&tournaments_home_team_first_board=') WHERE parameters LIKE '%&home_team_first_board=%';
/* 2026-08-11-11 */	UPDATE categories SET parameters = REPLACE(parameters, '&lineup_before_round_mins=', '&tournaments_lineup_before_round_mins=') WHERE parameters LIKE '%&lineup_before_round_mins=%';
/* 2026-08-11-12 */	UPDATE categories SET parameters = REPLACE(parameters, '&pgn_bulletin_file_template=', '&tournaments_pgn_bulletin_file_template=') WHERE parameters LIKE '%&pgn_bulletin_file_template=%';
/* 2026-08-11-13 */	UPDATE tournaments SET urkunde_parameter = REPLACE(urkunde_parameter, '&medaillenspiegel=', '&tournaments_medal_table=') WHERE urkunde_parameter LIKE '%&medaillenspiegel=%';
/* 2026-08-11-14 */	UPDATE tournaments SET urkunde_parameter = REPLACE(urkunde_parameter, '&statistik=', '&tournaments_statistics=') WHERE urkunde_parameter LIKE '%&statistik=%';
