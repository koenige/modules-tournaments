...
/* 2026-07-27-1 */	INSERT INTO categories (`category`, `description`, `main_category_id`, `path`, `parameters`, `sequence`, `last_update`) SELECT 'Tournaments', NULL, NULL, 'tournaments', '&alias=tournaments&use_subtree=1', NULL, NOW() FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM categories WHERE path = 'tournaments');
...
/* 2026-07-27-4 */	INSERT INTO categories (`category`, `description`, `main_category_id`, `path`, `parameters`, `sequence`, `last_update`) SELECT 'Status', NULL, (SELECT category_id FROM categories c WHERE path = 'tournaments'), 'tournaments/status', '&alias=tournaments/status&own_type_category=1&form_display=set&min_records=1&max_records=10', NULL, NOW() FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM categories WHERE path = 'tournaments/status');
/* 2026-07-27-5 */	RENAME TABLE `turniere_status` TO `tournaments_categories`;
/* 2026-07-27-6 */	ALTER TABLE `tournaments_categories` CHANGE `turnier_status_id` `tournament_category_id` int unsigned NOT NULL AUTO_INCREMENT, CHANGE `status_category_id` `category_id` int unsigned NOT NULL;
/* 2026-07-27-7 */	ALTER TABLE `tournaments_categories` ADD `property` varchar(255) COLLATE 'utf8mb4_unicode_ci' DEFAULT NULL AFTER `category_id`, ADD `type_category_id` int unsigned NULL AFTER `property`, ADD `sequence` tinyint unsigned NULL AFTER `type_category_id`, ADD `last_update` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `sequence`;
/* 2026-07-27-8 */	UPDATE tournaments_categories SET type_category_id = (SELECT category_id FROM categories WHERE path = 'tournaments/status' LIMIT 1);
/* 2026-07-27-9 */	ALTER TABLE `tournaments_categories` MODIFY `type_category_id` int unsigned NOT NULL;
/* 2026-07-27-10 */	ALTER TABLE `tournaments_categories` DROP INDEX `turnier_id_status_kategorie_id`, ADD UNIQUE `tournament_id_category_id` (`tournament_id`,`category_id`), ADD INDEX `category_id` (`category_id`), ADD INDEX `type_category_id` (`type_category_id`);
/* 2026-07-27-11 */	UPDATE _relations SET `detail_table` = 'tournaments_categories', `detail_id_field` = 'tournament_category_id' WHERE `detail_table` = 'turniere_status';
/* 2026-07-27-12 */	UPDATE _relations SET `detail_field` = 'category_id' WHERE `detail_table` = 'tournaments_categories' AND `detail_field` = 'status_category_id';
/* 2026-07-27-13 */	INSERT INTO _relations (`master_db`, `master_table`, `master_field`, `detail_db`, `detail_table`, `detail_id_field`, `detail_field`, `delete`) SELECT (SELECT DATABASE()), 'categories', 'category_id', (SELECT DATABASE()), 'tournaments_categories', 'tournament_category_id', 'type_category_id', 'no-delete' FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM _relations WHERE `detail_table` = 'tournaments_categories' AND `detail_field` = 'type_category_id');
